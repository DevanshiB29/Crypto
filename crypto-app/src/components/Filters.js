...
import { CryptoContext } from '../context/CryptoContext'; // Adjust path as needed
...
const Filters = () => {
  ...
  const handleCurrencySubmit = (e) => {
    ...
    setCurrency(val);
    currencyRef.current.value = ""; // Reset input field
  };
    ...
    <div className='w-full h-12 border-2 border-gray-100 rounded-lg flex items-center justify-between relative'>
      ...
      <div className='flex mr-7'>
        <form className='relative flex items-center font-nunito mr-12' onSubmit={handleCurrencySubmit}>
          <label htmlFor="currency" className='relative flex justify-center items-center mr-2 font-bold'>
            Currency:
          </label>
          <input
            id="currency"
            ...
            ref={currencyRef}
            placeholder='USD'
            className='w-16 rounded bg-gray-200 placeholder:text-gray-400 pl-2 required outline-0 border border-transparent focus:border-cyan leading-4'
          />
      ...
      <button className='w-[2rem] ml-4 hover:scale-110 transition-all
      transition-easse absolute right-0 top-0'
      onClick={resetFunction}>
        <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" className='w-full h-full fill-cyan'>
